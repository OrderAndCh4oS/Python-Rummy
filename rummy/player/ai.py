...
class AI(Player):
    ...
    def choose_to_discard_or_pick_up(self):
        if self.round.deck.has_discard():
            if self.ai_only:
                ...
                View.render_ai_thought(self, 'Choosing pick up')
            else:
                View.render_ai_turn_start(self)
            self.choose_pick_up()
        else:
            if self.ai_only:
                View.render_turn_start(self)
                ...
                View.render_ai_turn_start(self)
            View.render_ai_thought(self, 'Drawing first card from deck')
    ...
    def choose_pick_up(self):
        ...
        if min(scores) < current_score - 4 or min(scores) <= 10:
            View.render_ai_thought(self, 'Drawing from discard')
            self.hand.draw_card(self.round.deck.take_discard())
        else:
            View.render_ai_thought(self, 'Drawing from deck')
            self.hand.draw_card(self.round.deck.take_card())

    def discard_or_knock(self):
        if self.ai_only:
            View.render_ai_turn_end(self)
        View.render_ai_thought(self, 'Choosing card to discard')
